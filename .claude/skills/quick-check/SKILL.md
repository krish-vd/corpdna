...
Example: /quick-check Infosys
Example: /quick-check HDFC Bank"

Otherwise, invoke the `corpdna` agent with the company name `$ARGUMENTS`
and ask it to run a condensed analysis. The agent should return only:
...
Label the output "QUICK CHECK" clearly. Tell the user to run `/analyze`
for the full five-dimension report.
